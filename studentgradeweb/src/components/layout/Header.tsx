...
const Header = () => {
    ...
    useEffect(() => {
        ...
        setShouldShowHeader(!isAuthPage);

        // Add/remove margin to body based on header visibility
        const mainElement = document.querySelector('main');
        if (mainElement) {
            if (!isAuthPage) {
                mainElement.style.marginTop = '4rem';
            } else {
                        ...
                        <div className={`${isOpen ? 'block' : 'hidden'} xl:flex xl:items-center xl:space-x-8`}>
                            <div className="flex flex-col xl:flex-row xl:space-x-4">
                                ...
                                {(user && user.role !== "ROLE_ADMIN") && <>
                                    ...
                                    {(user.role === "ROLE_STUDENT") &&
                                        <Link href="/orientation" className="px-3 py-2 hover:text-primary">
                                            {t('career-orientation')}
                                        </Link>}
                                </>}
                            </div>
