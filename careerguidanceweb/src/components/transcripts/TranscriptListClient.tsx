...
export default function TranscriptListClient() {
    ...
    const loadTranscripts = useCallback(async () => {
        try {
            ...
            const res = await authApis().get<{ transcripts: PageResponse }>(url);

            if (res.data && res.data.transcripts && res.data.transcripts.content && Array.isArray(res.data.transcripts.content)) {
                setTranscripts(res.data.transcripts.content);
                setTotalPages(res.data.transcripts.totalPages || 1);
            } else {
                setTranscripts([]);
                setTotalPages(1);
            }
        } catch (ex: unknown) {
            console.error("Failed to load transcripts:", ex);
            setTranscripts([]);
            ...
            {!loading && !error && transcripts.length === 0 && (
                <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-12 text-center flex flex-col items-center justify-center max-w-2xl mx-auto animate-in fade-in">
                    ...
                    <h3 className="text-lg font-bold text-gray-900 mb-2">
                        {capitalizeFirstWord(t("none-transcripts"))}
                    </h3>
                    <p className="text-gray-500 text-sm max-w-sm">
                        {t('no-transcripts-available')}
                    </p>
                </div>
            )}

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative">
                {Array.isArray(transcripts) && transcripts.map((tItem, idx) => (
                    <div
                        key={tItem.id || idx}
                    ...
                    >
                        {/* Top Pattern Decor */}
                        <div className="h-2 w-full bg-linear-to-r from-blue-500 to-indigo-500 rounded-t-2xl"></div>
                            ...
                            <div className="grid grid-cols-2 gap-3 mt-6">
                                ...
                                {user?.role === "Teacher" && (
                                    <button
                                        ...
                                        className="flex justify-center items-center gap-2 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-600 shadow-lg shadow-gray-200 hover:shadow-blue-200 transition-all text-xs"
                                    >
                ...
                <div className="flex justify-center mt-10 gap-2">
                    {[...Array(totalPages)].map((_, i) => (
                        <button
                            key={i + 1}
                            onClick={() => setPage(i + 1)}
                            className={`min-w-[40px] h-10 px-3 rounded-xl font-medium text-sm transition-all focus:outline-none focus:ring-2 focus:ring-blue-500/20 ${page === i + 1
